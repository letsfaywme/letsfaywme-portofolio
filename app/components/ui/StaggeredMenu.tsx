...
const StaggeredMenu = ({
  ...
  const hamburgerTweenRef = useRef<gsap.core.Tween[]>([]);
  ...
  const animateHamburger = useCallback((opening: boolean) => {
    ...
    hamburgerTweenRef.current.forEach(t => t?.kill());
    hamburgerTweenRef.current = [];
    if (opening) {
      hamburgerTweenRef.current.push(
        gsap.to(top, { rotate: 45, y: 5, duration: 0.35, ease: 'power3.inOut', overwrite: 'auto' }),
        gsap.to(mid, { opacity: 0, scaleX: 0, duration: 0.2, ease: 'power2.out', overwrite: 'auto' }),
        gsap.to(bot, { rotate: -45, y: -5, duration: 0.35, ease: 'power3.inOut', overwrite: 'auto' }),
      );
    } else {
      hamburgerTweenRef.current.push(
        gsap.to(top, { rotate: 0, y: 0, duration: 0.3, ease: 'power3.inOut', overwrite: 'auto' }),
        gsap.to(mid, { opacity: 1, scaleX: 1, duration: 0.25, ease: 'power2.out', delay: 0.1, overwrite: 'auto' }),
        gsap.to(bot, { rotate: 0, y: 0, duration: 0.3, ease: 'power3.inOut', overwrite: 'auto' }),
      );
    }
  }, []);
    ...
    >
      ...
      >
        <span ref={iconRef} className="sm-hamburger" aria-hidden="true">
          <span ref={hamburgerTopRef} className="sm-hamburger-line" />
          <span ref={hamburgerMidRef} className="sm-hamburger-line" />
          <span ref={hamburgerBotRef} className="sm-hamburger-line" />
        </span>
      </button>
